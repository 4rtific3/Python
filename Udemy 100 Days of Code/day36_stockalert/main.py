...
import smtplib
# MIMEText function allows users to send UTF-8 encoded messages as SMTP only supports ASCII
...
av_response = requests.get("https://www.alphavantage.co/query", params=aa_parameters)
...
if abs(percent_delta) >= 5:
    ...
    msg["Subject"] = Header(email_content, "utf-8")

    with smtplib.SMTP(host="smtp.gmail.com") as connection:
        connection.starttls()
        connection.login(user=MY_EMAIL, password=GMAIL_PASSWORD)
        connection.sendmail(from_addr=MY_EMAIL, to_addrs=MY_EMAIL, msg=f"Subject: TSLA Alert\n\n{msg.as_string()}")
